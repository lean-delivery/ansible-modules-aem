...
class AEMBundle(object):
    ...
    def do_action(self):
        aem_request = requests.post('%s/system/console/bundles/%s' % (self.url, self.name),
                                    data={'action': self.action },
                                    auth=(self.admin_user, self.admin_password))
        if aem_request.status_code != 200:
            self.module.fail_json(msg='failed to perform %s action on %s bundle - %s' % 
                                  (self.action, aem_request.status_code, aem_request.json))
        self.changed = True
        self.msg.append('action %s was performmed on bundle %s' % (self.action, self.name))
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            action=dict(default='start', type='str', choices=['start', 'stop', 'refresh']),
